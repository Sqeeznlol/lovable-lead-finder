  ...
  if (compact) {
    return (
      <div className="flex items-center gap-2">
        <span className={`rounded-md border px-1.5 py-0.5 text-[10px] font-bold ${TIER_STYLE[tier]}`}>{tier}</span>
    ...
    <div className="rounded-2xl border bg-card/60 p-4 space-y-3">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Badge variant="outline" className="text-[10px] cursor-help">{r.confidence}</Badge>
            </TooltipTrigger>
        ...
        <div className={`flex h-12 w-12 shrink-0 flex-col items-center justify-center rounded-xl border-2 ${TIER_STYLE[tier]}`}>
          ...
          <span className="text-[8px] font-medium opacity-70">TIER {tier}</span>
        </div>
      </div>
      ...
      {r.killer.length > 0 && (
        <div className="flex flex-wrap items-center gap-1.5 rounded-xl bg-amber-500/10 px-3 py-2">
          ...
          {r.killer.map(k => (
            <span key={k} className="text-[11px] font-medium text-amber-700 dark:text-amber-400">{k}</span>
          ))}
        </div>
      )}
